...
4. If the repository uses `docs/spectacula`, store the canonical spec at `docs/spectacula/specs/<slug>.md` and keep exactly one JSON manifest for the current stage in `docs/spectacula/specs`, `ready`, `inprogress`, or `done`. The manifest must point back to the canonical spec and carry summary, next action, history, and resume context.
